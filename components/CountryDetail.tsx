...
export default function CountryDetail({ slug }: CountryDetailProps) {
  ...
  //     return countries.filter(country => !country.numericCode).map(country => ({
  //       name: country.name
  //     }));
  //   };
              ...
              <Card
                ...
                <CardContent className='flex gap-8 ' >
                  ...
                  <CardContent className='flex flex-col gap-1 text-sm'>
                    ...
                    <Box component={"div"} >
                      Currencies:  {e.currencies?.map(item => <Typography key={Number(e.numericCode)} variant='body2'>{`${item.name}, `}  </Typography>)}
                    </Box>
                    <Box component={"div"} >
                      Languages:   {e.languages.map(item => <Typography key={Number(e.numericCode)} variant='body2'>{`${item.name}, `}  </Typography>)}
                    </Box>
                  </CardContent>
                </CardContent>
